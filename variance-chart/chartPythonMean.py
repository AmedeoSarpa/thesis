"""
Graph to print the variance chart and compute standard deviation and mean for each sp category
"""
...
class UserStory :
       ...
       def checkEstimation(self,std,mean) :
              if self.effort > mean+std :
...
if __name__ == '__main__' :
       ...
       mean_list = [] #mean values for each category
       category_list = []
       for sp in category.keys() :
              if len(category[sp]) != 0 : 
                     print('STORY POINTS CATEGORY:',sp) 
                     current_mean = np.mean(category[sp])
                     current_std_dev = np.std(category[sp])
                     print('std deviation:',current_std_dev)
                     print('mean:',current_mean)
                     print()
                     ax.vlines(x=sp,ymin=current_mean-current_std_dev, ymax=current_mean+current_std_dev,colors='green')
       ...
       for story in story_list :
              mean = np.mean(category[story.sp])
              std = np.std(category[story.sp])
              if story.checkEstimation(std,mean) == True :
       ...
       ax.plot(category_list,mean_list,'ro',label="category's mean")
       ...
       fig.savefig("variance_chart_mean.png")
       plt.show()
