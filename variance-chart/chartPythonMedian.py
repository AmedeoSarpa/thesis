...
class UserStory:
    ...
    def checkEstimation(self, std, median):
        if self.effort > median+std:
            ...
            print("User story - ", self.title, ' - of sprint',
                  self.sprint, 'was overestimated, estimation was:', self.sp)
            return True
        if self.effort < median-std:
            self.bad_estimated = True
            print("User story - ", self.title, 'of sprint', self.sprint,
                  'was underestimated, estimation was:', self.sp)
            return True
        return False
...
if __name__ == '__main__':
    ...
    for line in fp.readlines()[1:]:
        if bool(sprint_pattern.match(line)):
            ...
            story_list.append(UserStory(vec[0], int(
                vec[1]), int(vec[2]), current_sprint))
    ...
    for sp in category.keys():
        if len(category[sp]) != 0:
            ...
            print()
            ax.vlines(x=sp, ymin=current_median-current_std_dev,
                      ymax=current_median+current_std_dev, colors='green')
    ...
    print('Total bad estimated stories', number_of_bad_estimations,
          'over a total number of stories of:', total_number_of_stories)
